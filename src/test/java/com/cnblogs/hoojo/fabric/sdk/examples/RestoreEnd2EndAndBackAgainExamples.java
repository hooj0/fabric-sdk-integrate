 ...
 * @version 1.0
 */
public class RestoreEnd2EndAndBackAgainExamples {
    ...
    */
    ...
    private void runChannel(HFClient client, Channel channel, Organization org, final int money) {
        ...
        try {
        	...
        	ChaincodeID usedChaincodeID = null;
        	if (checkChaincode(channel, client, org, chaincodeID)) {
        		usedChaincodeID = chaincodeID;
        	} else if (checkChaincode(channel, client, org, chaincodeID_11)) {
        		usedChaincodeID = chaincodeID_11;
        	} else {
        		throw new AssertionError("没有安装或实例化任何Chaincode");
        	}
        	
        	out("最终使用chaincodeID：%s", usedChaincodeID);
        	
        	// 查询 余额
            queryAmount(client, channel, "" + (latestMount + money), usedChaincodeID, org);
            ...
            final ChaincodeID ccId = usedChaincodeID;
            //  chaincode 交易
            moveAmount(client, channel, usedChaincodeID, "25", changeContext ? org.getPeerAdmin() : null)
            .thenApply((BlockEvent.TransactionEvent transactionEvent) -> { // 交易成功
                try {
                    waitOnFabric();
                    
                    out("交易 25 元成功");
                    
                    queryAmount(client, channel, "" + (latestMount + 25 + money), ccId, org);

                    // 如果之前的版本是 1.0 就升级到 1.1
                    if (ccId.getVersion().equals(CHAIN_CODE_VERSION)) {
                    	// 安装 chaincode
                ...
                try {
                    ...
                    print("################# %s", transactionEvent.getType());
                    print("################# %s", transactionEvent.getBlockEvent().getBlock());
                    ...
                    for (Peer peer : channel.getPeers()) {
                        if (!checkInstalledChaincode(client, peer, chaincodeID_11)) {
                        ...
                        if (!checkInstantiatedChaincode(channel, peer, chaincodeID_11)) {
                    ...
                    //运行新的chaincode 进行转账
                    CompletableFuture<TransactionEvent>  future = moveAmount(client, channel, ccId, "50", changeContext ? org.getPeerAdmin() : null);
                ...
                try {
					return queryAmount(client, channel, "" + (latestMount + 100 + 25 + money), ccId, org);
				} catch (Exception e1) {
					e1.printStackTrace();
					return null;
				}
    ...
    private static boolean checkInstalledChaincode(HFClient client, Peer peer, ChaincodeID ccId) throws Exception {
        out("检查是否存在chaincode: %s, version: %s, peer: %s", ccId.getName(), ccId.getVersion(), peer.getName());
        
        List<ChaincodeInfo> list = client.queryInstalledChaincodes(peer);
        
        boolean found = false;
        for (ChaincodeInfo chaincodeInfo : list) {
        	print("Peer: %s 已安装chaincode：%s", peer.getName(), json(chaincodeInfo));
        	
            if (ccId.getPath() != null) {
                found = ccId.getName().equals(chaincodeInfo.getName()) && ccId.getPath().equals(chaincodeInfo.getPath()) && ccId.getVersion().equals(chaincodeInfo.getVersion());
                if (found) {
                    break;
                }
            }

            found = ccId.getName().equals(chaincodeInfo.getName()) && ccId.getVersion().equals(chaincodeInfo.getVersion());
            if (found) {
                break;
            }
        }
    ...
    private static boolean checkInstantiatedChaincode(Channel channel, Peer peer, ChaincodeID ccId) throws InvalidArgumentException, ProposalException {
        out("在通道：%s 检查是否实例化 chaincode: %s, version: %s, peer: %s", channel.getName(), ccId.getName(), ccId.getVersion(), peer.getName());
    ...
    private boolean checkChaincode(Channel channel, HFClient client, Organization org, ChaincodeID ccId) throws Exception {
		out("检查通道 %s 是否安装且实例化Chaincode： %s", channel.getName(), ccId.toString());
		
		// 设置对等节点用户上下文
        client.setUserContext(org.getPeerAdmin());

        for (Peer peer : channel.getPeers()) {
            if (checkInstalledChaincode(client, peer, ccId) && checkInstantiatedChaincode(channel, peer, ccId)) {
            	return true;
            }
        }
	...
	private void checkChannel(String channelName, Channel channel, HFClient client) throws Exception {
        ...
        print("GroupsMap.Orderer: %s", groupsMap.get("Orderer"));
        print("GroupsMap.Application: %s", groupsMap.get("Application"));
