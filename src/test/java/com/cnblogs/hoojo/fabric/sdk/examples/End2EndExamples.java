...
public class End2EndExamples extends ApplicationLogging {
	...
	private void sendTransaction2Orderer(Organization org, HFClient client, Channel channel, ChaincodeID chaincodeId, Collection<ProposalResponse> successful, int money) throws Exception {
        ...
        Object result = completableFuture.thenApply(transactionEvent -> { 
        	...
        	return invokeChaincode(client, org, channel, chaincodeId);
    		...
    		logger.info("调用 invoke 交易完成，txId：{}", testTxID);
		...
		try {
            ...
            for (ProposalResponse response : responses) {
                if (!response.isVerified() || response.getStatus() != Status.SUCCESS) {
                    ...
                    checkArgument(StringUtils.equals(payload, String.valueOf(300 + money)), "交易结果和查询结果不一致");
                }
            }
		} catch (Exception e) {
	 ...
	 */
	private TransactionEvent invokeChaincode(HFClient client, Organization org, Channel channel, ChaincodeID chaincodeId) throws RuntimeException {
